...
/**
 * Takes a string and expands the first wildcard '*' character with ascii
 * @param  {string} trial The base string to be used.
 * @return {Array} An array of possibilites.
 */
function expandWildcard (trial, position, add = true) {
  const WILDCARD_CHAR = '*'

  var splitTrial = trial.split(WILDCARD_CHAR)

  if (splitTrial.length === 1) {
    return [trial]
  }

  if (splitTrial.length === 2) {
    var addIndex = splitTrial[0].length
    var newTrial = splitTrial[0] + splitTrial[1]
    var addChar = true
    var expandedTrial = addAscii(newTrial, addIndex, addChar)
    return expandedTrial
  }

  if (splitTrial.length === 3) {
    var doubleExpandedTrial = []
    var addIndex = splitTrial[0].length
    var newTrial = splitTrial[0] + splitTrial[1] + WILDCARD_CHAR + splitTrial[2]
    var addChar = true
    var expandedTrial = addAscii(newTrial, addIndex, addChar)
    expandedTrial.forEach((trial, i) => {
      debug(trial, i)
      var nextExpandedTrial = addAscii(trial, addIndex, addChar)
      doubleExpandedTrial = doubleExpandedTrial.concat(nextExpandedTrial)
    })
    debug('length', doubleExpandedTrial.length)
    return doubleExpandedTrial
  }
 ...
 * @return {Array} An array of possibilites.
 */
function addAscii (trial, position, add = true) {
  const ret = []
...
function testTrial (trial, addresses, isUncompressed) {
  ...
  const isSuccessful = testAddress(address.toString(), addresses)
